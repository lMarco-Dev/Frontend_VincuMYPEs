...
describe('Flujo de Ejecución y Certificación', function() {
    let driver;
    ...
    it('6. Estudiante (Delegado) sube entregable en Workspace', async function() {
        try {
            // Estudiante ya está logueado en step 5
            await driver.get('http://localhost:5173/workspace');
            await driver.sleep(3000);
            
            // Find the project card. It should be the only workspace card
            ...
            // Verificar si el estudiante actual (Estudiante 2) es el delegado
            let esDelegado = false;
            try {
                // Si el botón "Subir entregable" existe, Estudiante 2 ganó el sorteo aleatorio
                await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Subir entregable')]")), 3000);
                esDelegado = true;
            } catch (e) {
                // Si no existe, Estudiante 1 ganó
                esDelegado = false;
            }
            ...
            if (!esDelegado) {
                console.log("Estudiante 2 NO es el delegado (sorteo al azar). Cambiando a Estudiante 1...");
                await logout();
                await driver.wait(until.elementLocated(By.name('email')), 5000).sendKeys(estEmail);
                await driver.findElement(By.name('password')).sendKeys(estPass);
                const btnLogin = await driver.findElement(By.css('button[type="submit"]'));
                ...
                const cardWorkspace1 = await driver.wait(until.elementLocated(By.xpath("//*[contains(text(), 'Abrir Workspace')]/ancestor::div[contains(@style, 'cursor: pointer')] | //span[contains(text(), 'Abrir Workspace')]")), 5000);
                ...
                console.log("Estudiante 2 ES el delegado (sorteo al azar).");
            }

            // Ahora sí, quien sea que esté logueado, es el delegado
            btnSubirEntregable = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Subir entregable')]")), 5000);
            await driver.executeScript("arguments[0].click();", btnSubirEntregable);
            await driver.sleep(1000);
            ...
            await textarea.sendKeys('Aquí está el entregable final. Saludos.');
            
            // Clic en Confirmar subida (en el modal)
            ...
            await driver.wait(until.elementLocated(By.xpath("//*[contains(text(), 'exitosamente') or contains(text(), 'correctamente')]")), 10000).catch(() => console.log('No toast found, continuing'));
            await driver.sleep(3000);
        } catch (error) {
            const image = await driver.takeScreenshot();
            fs.writeFileSync('error-step6.png', image, 'base64');
            console.log("📸 Screenshot guardado como error-step6.png");
            throw error;
        }
    });

    it('7. MYPE revisa y aprueba el entregable', async function() {
        await logout();
        ...
        try {
            const btnVerEntregables = await driver.wait(until.elementLocated(By.xpath("//a[contains(., 'Ver entregables')]")), 5000);
            await driver.executeScript("arguments[0].scrollIntoView(true);", btnVerEntregables);
            await driver.sleep(1000);
            await btnVerEntregables.click();
          } catch (e) {
            console.log("❌ FALLÓ la búsqueda de Ver entregables. Imprimiendo proyectos...");
            const debugDiv = await driver.findElement(By.id("debug-proyectos-mype"));
            const jsonText = await debugDiv.getAttribute("textContent");
            console.log("MYPE PROYECTOS RECIBIDOS:", jsonText);
            throw e;
          }
        await driver.sleep(2000);
        
        // Click en "Aprobar" el entregable
        const btnAprobarEntregable = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Aprobar')]")), 5000);
        await driver.executeScript("arguments[0].click();", btnAprobarEntregable);
        await driver.sleep(2000);
        
        // Dar click en Marcar proyecto como completado
        const btnCompletar = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Marcar proyecto como completado')]")), 5000);
        await driver.executeScript("arguments[0].click();", btnCompletar);
        await driver.sleep(1000);

        // Dar click en Confirmar
        const btnConfirmarCompletar = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Confirmar')]")), 5000);
    ...
    it('8. Estudiante visualiza certificado', async function() {
        ...
        const certificado = await driver.wait(until.elementLocated(By.xpath("//*[contains(text(), 'Certificado') or contains(text(), 'EMPRESA E2E SAC')]")), 5000);
        expect(certificado).to.exist;
        
        const image = await driver.takeScreenshot();
        fs.writeFileSync('evidencia-fase-ejecucion.png', image, 'base64');
        console.log("✅ Certificado emitido y visualizado exitosamente. Captura guardada.");
    });
});
