...
const DetalleProyectoPage = () => {
  ...
  const [ratingMype, setRatingMype] = useState(null);
  const [cargandoRating, setCargandoRating] = useState(false);

  // Dependencia primitiva: dispara cuando el ID pasa de undefined/null a un número real
  useEffect(() => {
    const id = Number(proyecto?.mypeUsuarioId) || null;
    if (!id) return;

    let cancelled = false;
    setCargandoRating(true);
    httpClient
      .get(`/calificaciones/usuarios/${id}/rating`)
      .then((res) => { if (!cancelled) setRatingMype(res.data); })
      .catch(() => {})
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1440px] mx-auto">
      ...
      <nav aria-label="Breadcrumb" className="flex text-xs text-slate-400 mb-6 items-center gap-2 font-medium">
        <button onClick={() => navigate('/proyectos')} className="hover:text-primary transition-colors flex items-center gap-1 font-bold">
          <ChevronLeft size={14} />
          Explorar Proyectos
        </button>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center text-white/95 text-sm font-semibold">
            ...
            {!cargandoRating && ratingMype && ratingMype.promedio !== null && ratingMype.cantidad > 0 && (
              <span className="ml-3 flex items-center gap-1 text-yellow-300 text-xs">
                ...
                {ratingMype.promedio.toFixed(1)} ({ratingMype.cantidad})
              </span>
            )}
            {!cargandoRating && ratingMype && (ratingMype.promedio === null || ratingMype.cantidad === 0) && (
              <span className="ml-3 text-white/60 text-xs italic">Sin calificaciones</span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6 pt-5 border-t border-white/10 text-xs text-white/90 font-medium">
            <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full backdrop-blur-sm border border-white/5">
              <MapPin size={13} className="text-white/60" /> Remoto
            </span>
            <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full backdrop-blur-sm border border-white/5">
              <Users size={13} className="text-white/60" /> {proyecto.cupos} Vacantes Disponibles
            </span>
            <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full backdrop-blur-sm border border-white/5">
              <Calendar size={13} className="text-white/60" /> Inicio: {proyecto.fechaInicio || 'Por definir'}
            </span>
            <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full backdrop-blur-sm border border-white/5">
              <Clock size={13} className="text-white/60" /> {renderDuracion(proyecto).label}: {renderDuracion(proyecto).value}
            </span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="lg:col-span-2 space-y-6">
          <section className="bg-surface-container-lowest p-8 lg:p-10 rounded-[2.5rem] border border-outline-variant/60 shadow-sm space-y-8">
            <div>
              ...
              <p className="text-slate-600 leading-relaxed whitespace-pre-line text-sm font-medium">{proyecto.descripcion}</p>
            </div>
            ...
            {proyecto.objetivo && (
              <div className="pt-6 border-t border-outline-variant/20">
                ...
                <p className="text-slate-600 text-sm leading-relaxed font-medium whitespace-pre-line">{proyecto.objetivo}</p>
              </div>
            )}
              ...
              <div className="pt-6 border-t border-outline-variant/20">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {reqList.map((req, idx) => {
                    ...
                    return (
                      <span key={idx} className={`px-3.5 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 border transition-all ${matches ? 'bg-emerald-50 text-emerald-700 border-emerald-100 shadow-sm' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-primary/20'}`}>
              ...
              <div className="pt-6 border-t border-outline-variant/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                  {entregablesList.map((entregable, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-4 rounded-2xl bg-surface-container-low border border-outline-variant/30 hover:border-primary/20 transition-all group cursor-default">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 space-y-6">
            <section className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/60 shadow-md">
              ...
              <p className="text-sm text-slate-500 font-medium mb-6 leading-normal">Postula hoy mismo agregando tu mensaje de presentación. La empresa recibirá tu perfil de forma inmediata.</p>
              <div className="bg-slate-50 p-4 rounded-2xl mb-6 border border-slate-100">
                <div className="flex items-start gap-3">
                  <Calendar className="text-[#1e3a5f] mt-0.5" size={18} />
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{renderDuracion(proyecto).label}</p>
                    <p className="text-sm font-extrabold text-[#1e3a5f] mt-0.5">{renderDuracion(proyecto).value}</p>
                  </div>
                </div>
              </div>
              ...
              {haSuperadoLimite && !yaPostulo && (
                <div className="mt-4 p-3.5 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-amber-700 font-semibold leading-relaxed mt-0.5">Tienes {proyectosActivos.length} de {limiteProyectos} proyecto(s) activo(s). Solicita un incremento al administrador o finaliza tu trabajo actual para volver a postular.</p>
                  </div>
                </div>
              )}
              <p className="text-[10px] text-center text-slate-400 mt-4 uppercase tracking-widest font-bold">Convocatoria Gratuita para Estudiantes</p>
            </section>

            {/* Sobre la empresa (con RatingDisplay de respaldo) */}
            <section className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/60 shadow-sm">
              ...
              <button onClick={() => navigate('/proyectos')} className="text-xs font-bold text-primary hover:underline flex items-center gap-1 transition-colors">
                Explorar otros proyectos de la empresa <ChevronLeft size={14} className="rotate-180" />
              </button>
            </section>
          </div>
        </div>
