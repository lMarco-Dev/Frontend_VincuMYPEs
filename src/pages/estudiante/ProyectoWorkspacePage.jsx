...
import {
  ...
  Vote, Crown, Shield, Users, Building2, Eye
} from 'lucide-react';
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: `0.5px solid ${C.border}`, padding: '20px 28px', marginBottom: 20,
      }}
    >
...
const HeroRing = ({ value = 0, max = 1, size = 88 }) => {
  ...
  const sw = 7, r = (size - sw) / 2, circ = 2 * Math.PI * r, offset = circ - (pct / 100) * circ, id = 'multiRing';
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: 'rotate(-90deg)' }}>
        <defs><linearGradient id={id} x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#ef4444" /><stop offset="33%" stopColor="#eab308" /><stop offset="66%" stopColor="#10b981" /><stop offset="100%" stopColor="#3b82f6" /></linearGradient></defs>
...
const EstadoBadge = ({ estado }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 20, fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', background: cfg.bg, color: cfg.color, border: `0.5px solid ${cfg.color}30` }}>
...
const EntregableCard = ({ titulo, entregable, onUpload, onDownload, onDelete, index, esDelegado, esProyectoIndividual }) => {
  ...
  const confirmDelete = () => { if (onDelete && entregaId) onDelete(entregaId); setShowDeleteConfirm(false); };
  ...
  const handleUploadClick = () => {
    if (!puedeSubir) { alert('Solo el delegado del equipo puede subir entregables.'); return; }
    onUpload(titulo);
  };
      ...
      <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${hovered ? `${C.primary}30` : C.border}`, overflow: 'hidden', transition: 'all 0.3s', height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        <div style={{ padding: '14px 16px', flex: 1 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {entregable.subidoPorNombre && (
                      <span style={{ fontSize: 8, fontWeight: 600, background: '#fef3c7', color: '#92400e', padding: '1px 6px', borderRadius: 6, border: '0.5px solid #fbbf24', whiteSpace: 'nowrap' }}>📎 {entregable.subidoPorNombre}</span>
                    )}
                  </div>
                  {entregable.fechaSubida && (
                    <p style={{ fontSize: 8, color: C.textMuted, margin: '2px 0 0' }}>{formatDistanceToNow(new Date(entregable.fechaSubida), { addSuffix: true, locale: es })}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
            {tieneArchivo ? (
              <>
                <button onClick={handleViewFile} style={{ flex: 1, padding: '7px', borderRadius: 8, border: `0.5px solid ${C.border}`, background: '#fff', color: C.primary, fontSize: 10, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <Eye size={12} /> Ver
                </button>
                <button onClick={() => onDownload(entregable.archivoUrl || entregable.archivo, entregable.archivoNombre)} style={{ flex: 1, padding: '7px', borderRadius: 8, border: `0.5px solid ${C.border}`, background: '#fff', color: C.textSecondary, fontSize: 10, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <Download size={12} /> Descargar
                </button>
                {puedeSubir ? (
                  <button onClick={() => onUpload(titulo)} style={{ padding: '7px 10px', borderRadius: 8, border: `0.5px solid ${C.border}`, background: '#f8fafc', cursor: 'pointer' }} title="Actualizar archivo"><RefreshCw size={12} color={C.textMuted} /></button>
                ) : (
                  <button disabled style={{ padding: '7px 10px', borderRadius: 8, border: `0.5px solid ${C.border}`, background: '#f1f5f9', cursor: 'not-allowed', opacity: 0.5 }} title="Solo el delegado puede actualizar"><RefreshCw size={12} color="#9ca3af" /></button>
                )}
              </>
            ) : (
              puedeSubir ? (
                <button onClick={handleUploadClick} style={{ width: '100%', padding: '9px', borderRadius: 8, border: 'none', background: C.primary, color: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <Upload size={14} /> Subir entregable
                </button>
              ) : (
                <button disabled style={{ width: '100%', padding: '9px', borderRadius: 8, border: 'none', background: '#e2e8f0', color: '#94a3b8', fontSize: 11, fontWeight: 600, cursor: 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} title="Solo el delegado puede subir entregables">
...
export function ProyectoWorkspacePage() {
  ...
  useEffect(() => { if (activeTab === 'chat' && chatEndRef.current && mensajes.length > 0) { chatEndRef.current.scrollIntoView({ behavior: 'smooth' }); } }, [mensajes, activeTab]);
  useEffect(() => { const handler = (e) => { if (emojiPickerRef.current && !emojiPickerRef.current.contains(e.target)) setShowEmoji(false); }; document.addEventListener('mousedown', handler); return () => document.removeEventListener('mousedown', handler); }, []);

  const handleEnviarMensaje = async (e) => { e.preventDefault(); if (!nuevoMensaje.trim()) return; try { await enviarMensaje({ conversacionId, mensaje: nuevoMensaje.trim() }); setNuevoMensaje(''); setShowEmoji(false); } catch (err) { console.error('Error al enviar mensaje:', err); } };

  const handleSubirEntregable = async (e) => {
    e.preventDefault();
    if (!esDelegado && !esProyectoIndividual) { setUploadError('Solo el delegado del equipo puede subir entregables'); return; }
    ...
    try {
      ...
      setTimeout(() => { setShowUploadModal(false); setUploadSuccess(false); setUploadFile(null); setUploadDescripcion(''); setSelectedEntregable(null); resetUpload(); recargarWorkspace(); }, 1800);
    } catch (err) { setUploadError(err.response?.data?.message || err.message || 'Error al subir'); }
  };

  const handleEliminarEntregable = async (entregableId) => {
    if (!esDelegado && !esProyectoIndividual) { alert('Solo el delegado puede eliminar entregables'); return; }
    try { await eliminarEntregable(entregableId); recargarWorkspace(); } catch (err) { console.error('Error al eliminar entregable:', err); alert(err.response?.data?.message || 'Error al eliminar el entregable'); }
  };
  ...
  if (isLoading) {
    return (
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 60, height: 60, borderRadius: 20, background: `linear-gradient(135deg, ${C.primary}, ${C.purple})`, margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Loader2 size={28} color="#fff" /></motion.div>
    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          <div style={{ width: 70, height: 70, borderRadius: 20, background: '#fef2f2', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}><AlertCircle size={32} color="#dc2626" /></div>
      ...
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ background: '#0d1b35', borderRadius: 14, padding: '24px 28px', marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 20, padding: '4px 12px', fontSize: 9, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 10 }}>
            <motion.span animate={{ boxShadow: ['0 0 0 0 rgba(74,222,128,0.5)', '0 0 0 7px rgba(74,222,128,0)', '0 0 0 0 rgba(74,222,128,0)'] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 6, height: 6, borderRadius: '50%', background: '#4ade80', display: 'inline-block' }} /> Proyecto activo
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, color: 'rgba(255,255,255,0.4)', flexWrap: 'wrap' }}>
            ...
            {proyecto.fechaLimite && <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Calendar size={12} /> Límite: <span style={{ color: '#f87171', fontWeight: 600 }}>{format(new Date(proyecto.fechaLimite), "d 'de' MMMM, yyyy", { locale: es })}</span></span>}
            <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'inline-block' }} />
            <span>{proyecto.areaSistemas || 'Sistemas'}</span>
          </div>
        </div>
        <div style={{ position: 'relative', zIndex: 2 }}><HeroRing value={currentStats.completados} max={currentStats.total} size={88} /></div>
      </motion.div>
        ...
        <div style={{ padding: '24px 28px' }}>
          <AnimatePresence mode="wait">
              ...
              <motion.div key="entregables" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }}>
                ...
                {votacionCompletada && ganador && !esProyectoIndividual && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                    style={{ background: esDelegado ? 'linear-gradient(135deg, #ecfdf5, #d1fae5)' : 'linear-gradient(135deg, #eff6ff, #dbeafe)', border: esDelegado ? '1px solid #6ee7b7' : '1px solid #bfdbfe', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: esDelegado ? 'linear-gradient(135deg, #10b981, #059669)' : 'linear-gradient(135deg, #1B6FE8, #06B6D4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {esDelegado ? <Crown size={20} color="#fff" /> : <Shield size={20} color="#fff" />}
                    </div>
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{esDelegado ? 'Solo tú puedes subir los entregables del equipo.' : `Solo ${ganador?.estudianteNombre || 'el delegado'} puede subir entregables.`}</p>
                    </div>
                  </motion.div>
                )}
                {entregablesProyecto.length > 0 ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><div style={{ width: 4, height: 18, borderRadius: 2, background: C.primary, flexShrink: 0 }} /><h3 style={{ fontSize: 14, fontWeight: 700, color: C.textPrimary, margin: 0 }}>Lista de entregables</h3><span style={{ fontSize: 10, fontWeight: 600, color: C.textMuted, background: '#f1f5f9', padding: '3px 10px', borderRadius: 12 }}>{entregablesProyecto.length}</span></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 16 }}>
                      {entregablesProyecto.map((titulo, idx) => {
                        ...
                        return (
                          <EntregableCard key={`e-${idx}`} titulo={titulo} index={idx} entregable={entrega || null} 
                            esDelegado={esDelegado} esProyectoIndividual={esProyectoIndividual}
                            onUpload={(t) => { 
                              if (!esDelegado && !esProyectoIndividual) { alert('Solo el delegado puede subir entregables'); return; } 
                              setSelectedEntregable(t); setShowUploadModal(true); 
                            }}
                            onDownload={descargarArchivo} onDelete={handleEliminarEntregable} />
                        );
                      })}
                    </div>
                  </>
                ) : (
                  <div style={{ textAlign: 'center', padding: '60px 20px' }}><FileText size={48} color={C.textMuted} style={{ marginBottom: 16 }} /><h3 style={{ fontSize: 16, fontWeight: 700, color: C.textPrimary, marginBottom: 6 }}>No hay entregables definidos</h3><p style={{ fontSize: 13, color: C.textMuted }}>La MYPE aún no ha definido los entregables para este proyecto.</p></div>
                )}
              </motion.div>
            )}
            ...
            {activeTab === 'chat' && (
              <motion.div key="chat" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
                ...
                {esProyectoIndividual ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", height: "500px", background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", overflow: "hidden" }}>
                    <div style={{ padding: "14px 20px", borderBottom: "1px solid #e5e7eb", background: "#fff", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                      ...
                      <div><p style={{ fontSize: 13, fontWeight: 700, color: "#0F1F3D", margin: 0 }}>{mypeNombre}</p><p style={{ fontSize: 10, color: "#10b981", fontWeight: 600, margin: 0 }}>Chat directo</p></div>
                    </div>
                    <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", background: "#f8fafc" }}>
                      {mensajes.length === 0 ? (
                        <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ textAlign: "center" }}><MessageSquare size={36} color="#9ca3af" style={{ marginBottom: 10 }} /><p style={{ fontSize: 12, fontWeight: 600, color: "#9ca3af" }}>Sin mensajes aún</p><p style={{ fontSize: 11, color: "#9ca3af" }}>Envía un mensaje para coordinar</p></div></div>
                      ) : (
                        mensajes.map((msg, idx) => {
                          ...
                          return (
                            <React.Fragment key={msg.id || idx}>
                              {mostrarFecha && <div style={{ textAlign: "center", margin: "14px 0 10px" }}><span style={{ fontSize: 10, fontWeight: 600, color: "#9ca3af", background: "#fff", padding: "3px 12px", borderRadius: 12, border: "0.5px solid #e5e7eb" }}>{msg.fechaEnvio ? format(new Date(msg.fechaEnvio), "EEEE d 'de' MMMM", { locale: es }) : ''}</span></div>}
                              <motion.div initial={{ opacity: 0, x: isEstudiante ? 16 : -16, scale: 0.92 }} animate={{ opacity: 1, x: 0, scale: 1 }} style={{ display: "flex", justifyContent: isEstudiante ? "flex-end" : "flex-start", marginBottom: 6 }}>
                                <div style={{ maxWidth: "76%" }}>
                                  {!isEstudiante && <p style={{ fontSize: 10, fontWeight: 700, color: "#059669", margin: "0 0 2px 10px" }}>{mypeNombre}</p>}
                                  <div style={{ padding: "8px 13px", borderRadius: 12, background: isEstudiante ? "#dcf8c5" : "#fff", border: isEstudiante ? "none" : "0.5px solid #e5e7eb", borderTopRightRadius: isEstudiante ? 4 : 12, borderTopLeftRadius: isEstudiante ? 12 : 4 }}>
                                    <p style={{ fontSize: 13, color: "#0f1f3d", margin: 0, lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{msg.mensaje || msg.contenido}</p>
                                    <div style={{ display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end", marginTop: 4 }}><span style={{ fontSize: 9, color: "#9ca3af" }}>{hora}</span>{isEstudiante && <CheckCircle2 size={10} color="#9ca3af" />}</div>
                                  </div>
                                </div>
                              </motion.div>
                            </React.Fragment>
                    ...
                    <div style={{ padding: "12px 16px", borderTop: "1px solid #e5e7eb", background: "#fff", display: "flex", gap: 10, flexShrink: 0 }}>
                      <input type="text" value={nuevoMensaje} onChange={e => setNuevoMensaje(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleEnviarMensaje(e)} placeholder="Escribe un mensaje..." disabled={isEnviandoMensaje} style={{ flex: 1, padding: "10px 16px", borderRadius: 24, border: "0.5px solid #e5e7eb", background: "#f8fafc", fontSize: 13, outline: "none", color: "#0f1f3d", fontFamily: "inherit" }} />
                      <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }} onClick={handleEnviarMensaje} disabled={!nuevoMensaje.trim() || isEnviandoMensaje} style={{ width: 40, height: 40, borderRadius: "50%", border: "none", background: nuevoMensaje.trim() ? "#10b981" : "#e2e8f0", color: "#fff", cursor: nuevoMensaje.trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center" }}>{isEnviandoMensaje ? <Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> : <Send size={16} />}</motion.button>
                    </div>
                  </div>
                ) : (
                  /* ─── PROYECTO EN EQUIPO: Sidebar + Chat Grupal ─── */
                  <div style={{ display: "flex", height: "500px", background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                    <div style={{ width: 260, borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", flexShrink: 0, background: "#fff" }}>
                      <div style={{ padding: "14px 16px", borderBottom: "1px solid #f3f4f6" }}><h2 style={{ fontFamily: "Inter, Arial, sans-serif", fontSize: 13, fontWeight: 700, color: "#0F1F3D", margin: 0 }}>Conversaciones</h2></div>
                      <div style={{ flex: 1, overflowY: "auto" }}>
                        <button onClick={() => setChatTabActivo("EQUIPO")} style={{ width: "100%", padding: "12px 16px", background: chatTabActivo === "EQUIPO" ? "#eff6ff" : "transparent", border: "none", borderBottom: "1px solid #f9fafb", borderLeft: chatTabActivo === "EQUIPO" ? "3px solid #1B6FE8" : "3px solid transparent", cursor: "pointer", textAlign: "left", transition: "all 0.15s", display: "flex", alignItems: "center", gap: 10 }}
                          onMouseEnter={(e) => { if (chatTabActivo !== "EQUIPO") e.currentTarget.style.background = "#f9fafb"; }} onMouseLeave={(e) => { if (chatTabActivo !== "EQUIPO") e.currentTarget.style.background = "transparent"; }}>
                          <div style={{ width: 36, height: 36, borderRadius: 10, background: chatTabActivo === "EQUIPO" ? "linear-gradient(135deg, #1B6FE8, #06B6D4)" : "linear-gradient(135deg, #EFF6FF, #DBEAFE)", border: chatTabActivo === "EQUIPO" ? "2px solid #1B6FE8" : "2px solid #BFDBFE", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Users size={16} color={chatTabActivo === "EQUIPO" ? "#fff" : "#1D4ED8"} /></div>
                          <div style={{ flex: 1, minWidth: 0 }}><p style={{ fontFamily: FONT, fontSize: 12, fontWeight: 600, color: "#0F1F3D", margin: 0 }}>Chat de Equipo</p><p style={{ fontFamily: FONT, fontSize: 10, color: "#9CA3AF", margin: "2px 0 0" }}>Solo estudiantes</p></div>
                        </button>
                        <button onClick={() => setChatTabActivo("PROYECTO")} style={{ width: "100%", padding: "12px 16px", background: chatTabActivo === "PROYECTO" ? "#eff6ff" : "transparent", border: "none", borderBottom: "1px solid #f9fafb", borderLeft: chatTabActivo === "PROYECTO" ? "3px solid #1B6FE8" : "3px solid transparent", cursor: "pointer", textAlign: "left", transition: "all 0.15s", display: "flex", alignItems: "center", gap: 10 }}
                          onMouseEnter={(e) => { if (chatTabActivo !== "PROYECTO") e.currentTarget.style.background = "#f9fafb"; }} onMouseLeave={(e) => { if (chatTabActivo !== "PROYECTO") e.currentTarget.style.background = "transparent"; }}>
                          <div style={{ width: 36, height: 36, borderRadius: 10, background: chatTabActivo === "PROYECTO" ? "linear-gradient(135deg, #7C3AED, #A855F7)" : "linear-gradient(135deg, #F5F3FF, #EDE9FE)", border: chatTabActivo === "PROYECTO" ? "2px solid #7C3AED" : "2px solid #DDD6FE", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Building2 size={16} color={chatTabActivo === "PROYECTO" ? "#fff" : "#7C3AED"} /></div>
                          <div style={{ flex: 1, minWidth: 0 }}><p style={{ fontFamily: FONT, fontSize: 12, fontWeight: 600, color: "#0F1F3D", margin: 0 }}>Chat del Proyecto</p><p style={{ fontFamily: FONT, fontSize: 10, color: "#9CA3AF", margin: "2px 0 0" }}>Equipo + {mypeNombre}</p></div>
                        </button>
                      </div>
                      <div style={{ padding: "10px 16px", borderTop: "1px solid #f3f4f6", textAlign: "center" }}><span style={{ fontFamily: FONT, fontSize: 10, color: "#d1d5db" }}>2 conversaciones</span></div>
                    </div>
                    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                      {chatsGrupales?.length > 0 ? (
                        <ChatGrupalPanel proyectoId={proyectoId} chat={chatsGrupales.find(c => c.tipo === chatTabActivo)} />
                      ) : (
                        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "#f8fafc" }}><div style={{ textAlign: "center" }}><div style={{ width: 64, height: 64, borderRadius: 16, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}><MessageSquare size={28} color="#d1d5db" /></div><p style={{ fontFamily: FONT, fontSize: 15, fontWeight: 600, color: "#9CA3AF" }}>Chats no disponibles</p></div></div>
                      )}
                    </div>
                  </div>
                )}
            ...
            {!esProyectoIndividual && activeTab === 'votacion' && (
              <motion.div key="votacion" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} style={{ padding: '20px 0' }}>
                {isLoadingVotacion ? (
                  <div style={{ textAlign: 'center', padding: 40 }}><Loader2 size={32} className="animate-spin" color="#1B6FE8" /><p style={{ marginTop: 12, color: '#6b7280', fontSize: 13 }}>Cargando votación...</p></div>
                ) : votacionCompletada && ganador ? (
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200 }} style={{ width: 80, height: 80, borderRadius: '50%', background: 'linear-gradient(135deg, #fbbf24, #f59e0b)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}><Crown size={36} color="#fff" /></motion.div>
                    <h3 style={{ fontSize: 18, fontWeight: 800, color: '#0f1f3d', marginBottom: 4 }}>{ganador.estudianteNombre}</h3>
                    <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 8 }}>
                      {votacion.candidatos?.length === 2 ? <span>🎲 Elegido al azar por el sistema</span> : <span>🎉 Delegado del equipo · {ganador.votosRecibidos} voto{ganador.votosRecibidos !== 1 ? 's' : ''} recibido{ganador.votosRecibidos !== 1 ? 's' : ''}</span>}
                    </p>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 12, background: '#eff6ff', border: '1px solid #bfdbfe', color: '#1d4ed8', fontSize: 12, fontWeight: 600, marginBottom: 24 }}><Shield size={14} />Solo el delegado puede subir entregables</div>
                    <div style={{ maxWidth: 400, margin: '0 auto' }}>
                      {votacion.candidatos?.map((c, idx) => {
                        const pct = votacion.totalVotos > 0 ? Math.round((c.votosRecibidos / votacion.totalVotos) * 100) : 0;
                        return (
                          <div key={c.estudianteId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, background: c.esGanador ? '#fffbeb' : '#f9fafb', border: c.esGanador ? '2px solid #fbbf24' : '1px solid #e5e7eb', marginBottom: 8 }}>
                            <span style={{ width: 28, height: 28, borderRadius: '50%', background: c.esGanador ? 'linear-gradient(135deg, #fbbf24, #f59e0b)' : '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{idx + 1}</span>
                            <div style={{ flex: 1, textAlign: 'left' }}>
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontSize: 13, fontWeight: 600, color: '#0f1f3d' }}>{c.estudianteNombre}</span><span style={{ fontSize: 12, fontWeight: 700, color: '#6b7280' }}>{c.votosRecibidos} voto{c.votosRecibidos !== 1 ? 's' : ''}</span></div>
                              <div style={{ marginTop: 6, height: 4, borderRadius: 2, background: '#e5e7eb', overflow: 'hidden' }}><motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 1, ease: 'easeOut' }} style={{ height: '100%', borderRadius: 2, background: c.esGanador ? 'linear-gradient(90deg, #fbbf24, #f59e0b)' : '#9ca3af' }} /></div>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity }} style={{ width: 70, height: 70, borderRadius: '50%', background: 'linear-gradient(135deg, #1B6FE8, #06B6D4)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}><Vote size={32} color="#fff" /></motion.div>
                    <h3 style={{ fontSize: 17, fontWeight: 800, color: '#0f1f3d', marginBottom: 6 }}>🗳️ Votación en curso</h3>
                    <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 20 }}>Elige al delegado del equipo. Solo él podrá subir entregables.</p>
                    <button onClick={() => setShowVotacionModal(true)} style={{ padding: '12px 28px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #1B6FE8, #06B6D4)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 16px rgba(27,111,232,0.3)' }}><Vote size={16} />Ir a votar</button>
                    <div style={{ marginTop: 24, display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                      {votacion?.candidatos?.slice(0, 4).map((c) => (
                        <div key={c.estudianteId} style={{ padding: '12px 16px', borderRadius: 12, background: '#f9fafb', border: '1px solid #e5e7eb', textAlign: 'center', minWidth: 80 }}>
                          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #e0e7ff, #c7d2fe)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 6px', color: '#4f46e5', fontWeight: 700, fontSize: 13 }}>{c.estudianteNombre?.charAt(0)}</div>
                          <span style={{ fontSize: 11, color: '#6b7280' }}>{c.estudianteNombre?.split(' ')[0]}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                    <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}><Clock size={28} color="#9ca3af" /></div>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f1f3d', marginBottom: 6 }}>Votación no disponible</h3>
                    <p style={{ fontSize: 13, color: '#6b7280', maxWidth: 360, margin: '0 auto' }}>La votación de delegado se abrirá automáticamente cuando todos los cupos del proyecto estén confirmados.</p>
                  </div>
                )}
              </motion.div>
            )}
              ...
              <AnimatePresence mode="wait">
                {uploadSuccess ? (
                  ...
                  <motion.form key="form" onSubmit={handleSubirEntregable} style={{ padding: 24 }}>
                    <div style={{ marginBottom: 16 }}><label style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 7 }}>Descripción</label><textarea value={uploadDescripcion} onChange={e => setUploadDescripcion(e.target.value)} placeholder="Describe esta entrega..." rows={3} style={{ width: '100%', padding: '11px 13px', borderRadius: 12, border: `0.5px solid ${C.border}`, fontSize: 13, outline: 'none', resize: 'none', color: C.textPrimary, fontFamily: 'inherit', boxSizing: 'border-box' }} /></div>
                    <div onDragEnter={e => { e.preventDefault(); setDragActive(true); }} onDragOver={e => { e.preventDefault(); setDragActive(true); }} onDragLeave={e => { e.preventDefault(); setDragActive(false); }} onDrop={e => { e.preventDefault(); setDragActive(false); const file = e.dataTransfer.files?.[0]; if (file) { if (file.size > 15 * 1024 * 1024) setUploadError('Máx 15 MB'); else { setUploadFile(file); setUploadError(''); } } }} style={{ padding: '28px 20px', borderRadius: 14, textAlign: 'center', cursor: 'pointer', border: `2px dashed ${dragActive ? C.primary : uploadFile ? '#10b981' : C.border}`, background: dragActive ? '#eff6ff' : uploadFile ? '#ecfdf5' : '#fafaf8', transition: 'all 0.3s', marginBottom: 18 }}>
                      ...
                      {uploadFile ? <div><FileText size={32} color={C.primary} style={{ marginBottom: 10 }} /><p style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary, margin: '0 0 4px' }}>{uploadFile.name}</p><p style={{ fontSize: 11, color: C.textMuted, margin: '0 0 10px' }}>{(uploadFile.size / 1024 / 1024).toFixed(2)} MB</p><button type="button" onClick={() => setUploadFile(null)} style={{ fontSize: 11, fontWeight: 600, color: '#dc2626', border: 'none', background: 'none', cursor: 'pointer' }}>Cambiar archivo</button></div> : <label htmlFor="file-input" style={{ cursor: 'pointer' }}><Upload size={30} color={C.textMuted} style={{ marginBottom: 10 }} /><p style={{ fontSize: 13, fontWeight: 600, color: C.textSecondary, margin: '0 0 4px' }}>Arrastra tu archivo o haz clic</p><p style={{ fontSize: 11, color: C.textMuted, margin: 0 }}>PDF, DOCX, ZIP, JPG (Máx. 15 MB)</p></label>}
                    </div>
                    {isSubiendo && <div style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}><span style={{ fontSize: 10, fontWeight: 600, color: C.textMuted }}>Subiendo…</span><span style={{ fontSize: 10, fontWeight: 600, color: C.primary }}>{uploadProgress}%</span></div><div style={{ width: '100%', height: 6, borderRadius: 3, background: '#f1f5f9', overflow: 'hidden' }}><motion.div initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} transition={{ duration: 0.3 }} style={{ height: '100%', borderRadius: 3, background: `linear-gradient(90deg, ${C.primary}, ${C.purple})` }} /></div></div>}
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button type="submit" disabled={!uploadFile || isSubiendo} style={{ flex: 1, padding: '11px 16px', borderRadius: 12, border: 'none', background: !uploadFile ? '#e2e8f0' : C.primary, color: '#fff', fontSize: 13, fontWeight: 600, cursor: !uploadFile ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>{isSubiendo ? <><Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> Subiendo…</> : <><Send size={14} /> Subir entregable</>}</button>
                    </div>
                  </motion.form>
                )}
              </AnimatePresence>
