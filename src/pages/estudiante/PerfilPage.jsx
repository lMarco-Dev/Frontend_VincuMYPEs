...
const getSkillColor = (skill) => {
  ...
  if (
    ["react", "vue", "angular", "html", "css", "javascript", "typescript"].some(
      (k) => s.includes(k),
    )
  )
    return "bg-blue-50 text-blue-700 border-blue-100";
  if (["java", "spring", "kotlin"].some((k) => s.includes(k)))
    return "bg-orange-50 text-orange-700 border-orange-100";
  if (["python", "django", "flask", "fastapi"].some((k) => s.includes(k)))
    return "bg-yellow-50 text-yellow-700 border-yellow-100";
  if (["mysql", "postgres", "mongodb", "sql"].some((k) => s.includes(k)))
    return "bg-emerald-50 text-emerald-700 border-emerald-100";
  if (["docker", "git", "linux", "aws"].some((k) => s.includes(k)))
...
const PerfilPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updatePerfil(formData, { onSuccess: () => setIsEditing(false) });
  };
    ...
    return (
      <div className="p-12 flex items-center justify-center min-h-[400px]">
        <div className="text-slate-500 flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">Cargando perfil...</span>
        </div>
      </div>
    );
        ...
        <div className="px-4 md:px-8 -mt-14 relative flex flex-col md:flex-row items-end gap-4 md:gap-6">
          ...
          <div className="flex-1 pb-2">
            ...
            <p className="text-sm font-bold text-primary flex items-center gap-2 mt-0.5">
              <span className="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 text-[10px] font-extrabold uppercase">
                {displayRol}
              </span>
              {isEstudiante && (
          ...
          {isEstudiante && (
            <section className="bg-white p-6 lg:p-8 rounded-3xl shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 mb-5">
                <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center text-primary">
                  <User size={18} />
                </div>
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed mb-5 font-semibold">
                    {user.bio || (
                      <span className="italic text-slate-400">
                        Cuéntanos un poco sobre ti... (haz clic en "Editar
                        Perfil")
                      </span>
                    )}
                  </p>
                  <div className="flex items-start gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100">
                    ...
                    <div>
                      ...
                      <span className="text-sm font-bold text-slate-800">
                        {academicInfo.ciclo} · {academicInfo.carrera}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/40 text-sm resize-none"
                      rows="3"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/40 text-sm"
                  ...
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={isUpdating}
                      className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 disabled:opacity-50 text-sm"
                    >
                      {isUpdating ? (
                ...
                <div className="relative pl-12">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:-translate-y-0.5 transition-transform">
                    ...
                    <p className="text-xs text-slate-400 font-semibold">
                      Código: {academicInfo.codigo} · Ciclo:{" "}
                      {academicInfo.ciclo}
                    </p>
                  </div>
                </div>
            ...
            <section className="bg-white p-6 lg:p-8 rounded-3xl shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  ...
                  className="text-primary font-bold flex items-center gap-1 hover:underline text-xs"
                >
              ...
              {proyectosAceptados.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {proyectosAceptados.slice(0, 4).map((proyecto, index) => (
                    <Link
                      to={`/proyectos/${proyecto.proyectoId}`}
                      key={index}
                      className="group relative aspect-video rounded-2xl overflow-hidden cursor-pointer bg-slate-900 border border-slate-100"
                    >
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${index % 2 === 0 ? "from-[#1e3a5f] to-[#4648d4]" : "from-slate-700 to-slate-900"} group-hover:opacity-90 transition-opacity`}
                      />
                      <div className="absolute inset-0 flex flex-col justify-end p-4 z-10 bg-gradient-to-t from-black/80 to-transparent">
                        <h5 className="text-white font-bold text-sm truncate">
                          {proyecto.proyectoTitulo || "Proyecto"}
                        </h5>
                        <span className="text-[10px] font-bold text-emerald-300 flex items-center gap-1 mt-0.5">
                          <CheckCircle2 size={10} />
                          Aceptado
                        </span>
                      </div>
                ...
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 flex flex-col items-center">
                  <p className="text-slate-400 font-semibold text-sm mb-4 max-w-sm">
                    Aún no tienes proyectos completados. ¡Postula a proyectos
                    MYPE para armar tu portafolio!
                  </p>
                  <Link
                    to="/proyectos"
                    className="inline-flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:shadow-md transition-all active:scale-95"
                  >
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <section className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center text-center">
              ...
              <p className="text-xs text-slate-400 font-semibold">
                {completitud < 60
                  ...
                  : completitud < 100
                    ? "¡Casi completo! Agrega más datos"
                    : "¡Perfil completo! Las MYPEs te notarán"}
              </p>
              {completitud < 100 && (
                <button
          ...
          <section className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            ...
            <p className="text-sm font-bold text-slate-700">
              {locationInfo.ciudad}, {locationInfo.pais}
            </p>
            ...
            <div className="space-y-2">
              {user.linkedinUrl ? (
                ...
                >
                  ...
                  <ExternalLink
                    size={14}
                    className="text-[#0077B5]/40 group-hover:translate-x-0.5 transition-transform"
                  />
                </a>
              ) : (
                isEstudiante && (
                  <button
                    onClick={handleStartEdit}
                    className="flex items-center justify-between w-full p-3 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors group text-left"
                  >
                    <div className="flex items-center gap-2.5">
              ...
              <a
                ...
                className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-primary/5 transition-colors group"
              >
                <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                {user.skills ? (
                  user.skills.split(",").map((skill, index) => (
                    <span
                      key={index}
                      className={`px-3 py-1.5 rounded-full text-xs font-extrabold border ${getSkillColor(skill)}`}
                    >
                      {skill.trim()}
                    </span>
                  ))
                  ...
                  <button
                    onClick={handleStartEdit}
                    className="text-xs text-slate-400 italic hover:text-primary transition-colors font-semibold"
                  >
