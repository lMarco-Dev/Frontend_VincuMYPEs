...
import { ArrowRight, ArrowLeft, HelpCircle, CheckCircle2, Sparkles, Loader2 } from "lucide-react";
...
const ARBOL_DECISION = {
  ...
  infraestructura: {
    ...
    opciones: [
      ...
      { texto: "Voy a abrir o expandir un local y necesito saber qué equipos instalar", resultado: "4.2" },
...
export function CrearProyectoForm() {
  ...
  const handlePublish = async () => {
    ...
    try {
      // JSON listo para enviarse a Spring Boot mapeando las variables resultantes del árbol
      const projectPayload = {
        titulo: selectedResult.titulo,
        descripcion: selectedResult.recibiras + (comentario ? `\n\nNotas adicionales del negocio: ${comentario}` : ""),
        areaSistemas: selectedResult.areaSistemas,
        duracionEstimada: selectedResult.tiempo
      };
      
      console.log("Despachando proyecto clasificado al backend:", projectPayload);
      // Aquí invocas tu mutación de react-query, ej: mutate(projectPayload);
      
      // Simulamos respuesta exitosa por el momento
      setTimeout(() => {
        ...
        {!selectedResult ? (
          ...
          >
            <div className="flex items-center gap-2.5 mb-1">
              ...
              <p className="text-sm font-semibold text-slate-300 uppercase tracking-wider text-[11px]">
          ...
          /* ── MOSTRAR PROPUESTA FINAL DE PROYECTO ── */
          <motion.div
            key="resultado-final"
          ...
          >
            <div className="flex items-center gap-2 bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-lg w-fit">
              ...
              <span className="text-xs font-bold text-orange-400 uppercase tracking-wide">
                Proyecto Recomendado para tu MYPE
              </span>
            </div>
            ...
            <div className="bg-[#081828] border border-slate-700/50 rounded-xl p-5 flex flex-col gap-4 shadow-inner">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tu proyecto ideal:</p>
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Lo que recibirás:</p>
            ...
            {/* Caja de comentarios opcional para refinar detalles */}
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ¿Deseas agregar algún detalle o aclaración extra? (Opcional)
              </label>
              <textarea
                className="saas-textarea"
                value={comentario}
                onChange={(e) => setComentario(e.target.value)}
                placeholder="Ej: Deseo que el catálogo tenga colores pasteles debido a que mi negocio es una pastelería..."
              />
            </div>

            <div className="text-sm font-medium text-slate-300 mt-1">
              ¿Es esto lo que necesitas para resolver tu problema?
            </div>

            {/* Botonera de decisión final */}
            <div className="flex gap-3 pt-1">
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = "#F97316"; e.currentTarget.style.color = "#white"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(27, 111, 232, 0.25)"; e.currentTarget.style.color = "#94A3B8"; }}
              ...
              <button
                ...
                style={{ flex: 1, height: 48, borderRadius: 10, border: "none", color: "white", background: "linear-gradient(135deg,#F97316,#DC4A00)", cursor: "pointer", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "transform 0.2s", opacity: isSubmitting ? 0.7 : 1 }}
